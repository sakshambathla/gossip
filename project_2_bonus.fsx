...
type FailureSimulationMessage = bool
...
let failurePercenTage = args.[3] |> double
if failurePercenTage < 0.0 || failurePercenTage > 1.0 then
    failwith "failure percentage not in range"
    ...
    override this.OnReceive message = 
        ...
        | :? PushSumPropogationMessage as message ->
            ...
            if not converged then
                ...
                s <- double(s * 0.5)
                w <- double(w * 0.5) 
        | :? FailureSimulationMessage as message ->
            let (failure) = unbox<FailureSimulationMessage> message
            converged <- true
    ...
    override this.OnReceive message =
        match message with
            | :? InitSupervisorMessage as message -> 
                ...
                if failurePercenTage > 0.0 then
                    let nodestofail = ceil(failurePercenTage * (numberNodes |> double)) |> int
                    for i in 1..nodestofail do
                        let a = random.Next(0,numNodes)
                        allWorkers.[i] <! (true)
                    convergence <- nodestofail 
